  ...
  try {
    ...
    try {
      ...
      const response = await axios.post(
        `${userServiceUrl}/v1/ums/auth/validate`,
        { token },
        {
          headers: {
      ...
      if (!response.data.valid) {
        return next(
          new AppError("JsonWebTokenError", "Invalid or expired token", 401)
        );
      }
    ...
    if (error instanceof AppError) {
      ...
      next(
        new AppError(
          "ValidationError",
          "Authentication failed due to server issue",
          500
        )
      );
    }
...
const requireRole = (roles) => {
  return (req, res, next) => {
    ...
    if (!req.user) {
      return next(
        new AppError("ValidationError", "User not authenticated", 401)
      );
    }
