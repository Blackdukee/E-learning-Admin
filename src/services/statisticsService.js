...
const getActiveUsers = async (params = {}) => {
  try {
    const response = await axios.get(`${authServiceURL}/statistics/active-users`, { params });
    return response.data;
  } catch (error) {
    logger.error(`Error fetching active users statistics: ${error.message}`, error);
    throw new AppError('Failed to fetch active users statistics', 500);
  }
};
